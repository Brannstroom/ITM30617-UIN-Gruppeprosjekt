    ...
    <div className="fixed top-0 left-0 h-full w-72 bg-gray-800">
        ...
        <nav className="flex flex-col flex-1 mt-4 text-2xl text-gray-200  w-4/6 m-auto">
          ...
          <div  className="mt-5 ml-4">
            ...
            <button onClick={() => setStoreDropDown(!storeDropDown)} className="w-20 hover:text-gray-400">
              {storeDropDown ? <ChevronDown /> : <ChevronRight />}
            </button>
          </div>

          {(storeDropDown && categories) && categories.map((category) => (
            <a href={`/games/${category?.slug?.current}`} key={category.title} className={LinkStyleSub}>
              {category?.title}
            </a>
          ))}
